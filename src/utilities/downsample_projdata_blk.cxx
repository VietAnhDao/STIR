...
int main(int argc, char * argv[]){ 
    ...
    shared_ptr<ProjDataInfo> downsample_proj_data_info_sptr( 
                                                            ...
                                                            false)); 
    // now we cast the ProjDataInfo down to either ProjDataInfoCylindrical/ProjDataInfoBlocksOnCylindrical/ProjDataInfoGeneric. 
    ...
    {
        low_res_segments[seg] = new SegmentByView<elem_type>(low_res_proj_data_info_blk_ptr->get_empty_segment_by_view (seg));
    }
            ...
            { 
                ...
                { 
                    ...
                    {  
                        ...
                        { 
                            if(detA==detB && ringA==ringB){continue;}; 
                            ...
                            (*low_res_segments[low_res_b.segment_num()])[low_res_b.view_num()][low_res_b.axial_pos_num()][low_res_b.tangential_pos_num()] += ful_res_b.get_bin_value();
                        }
                    } 
                } 
            } 
    ...
    for (int seg=start_segment_index; seg<=end_segment_index; seg++)
    {
        std::cout << "writing seg: " << seg << "\n" ;
        tmp_sptr->set_segment(*low_res_segments[seg]);
    }
    std::cout << "finish writing segment to projdata\n";
    tmp_sptr->write_to_file("test.hs");
    return 0; 
} 
